...
% This case is essentially the same as example 07: The test case A1 from 3rd PIV challenge [ref. 6] is
% treated. Mean and rms velocities are computed, velocity PDF is determined and wavenumber spectra is
% calculated; these results can be compared to results shown in reference [6], Figs. 3a, 16, 18 and Table 12.
%
% Another shown feature is the possibility to add iterations during the processing. Initially, the data are
% processed in four passes (with IA size 64x64, 32x32, 16x16 and 8x8 pixels). Additional pass (with 8x8
% pixels interrogation size) are then performed. Effect of number of passes on velocity PDF and spectra is
% shown.
%
...
% *Instructions:*
%
% # Download images (case A2) from <http://www.pivchallenge.org/pub05/A/A1.zip PIVchallenge site>,
...
imagePath = ['../Data/Test PIVChallenge3A1'];  % folder with processed images
...
try
    ...
    error(['No images found. Please, download images (case A1) from http://www.pivchallenge.org/pub05/A/A1.zip, '...
        'unzip them and place them to folder ../Data/Test PIVChallenge3A1.']);
end
...
pivPar.qvPair = {'U','clipLo',-0.20,'clipHi',0.20,...
    'invLoc'};                            % plot locations of invalid velocity vectors
figure(1);
...
fprintf('Statistics (8x8 px, 2 passes): mean(U) = %+6.4f, mean(V) = %+6.4f, std(U) = %+6.4f, std(V) = %+6.4f\n',...
...
ylabel('PDF (a.u.)');
title('Velocity PDF - compare to Fig. 11 in [6]');
...
title('Velocity spectra - compare to Figs. 3a and 8 in [6]');
